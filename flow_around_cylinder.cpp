...
MatrixXd temp = MatrixXd::Zero(M + 1, N + 1);
...
mat_buf buf_pre_A(temp.data(), siz);
mat_buf buf_pre_B(temp.data(), siz);
mat_buf buf_pre_R(temp.data(), siz);
mat_buf buf_pre_O(temp.data(), siz);
mat_buf buf_pre_E(temp.data(), siz);

mat_buf buf_Xi_x(temp.data(), siz);
mat_buf buf_Xi_y(temp.data(), siz);
mat_buf buf_Eta_x(temp.data(), siz);
mat_buf buf_Eta_y(temp.data(), siz);

mat_buf buf_X_xi(temp.data(), siz);
mat_buf buf_X_eta(temp.data(), siz);
mat_buf buf_Y_xi(temp.data(), siz);
mat_buf buf_Y_eta(temp.data(), siz);
...
void push() {
  ...
  // pre_A pre_B pre_R pre_O pre_E Xi_x Xi_y Eta_x Eta_y U V
  ...
  auto tas = calc_queue.submit([&](sycl::handler& h) {
    auto pA = buf_pre_A.get_access<sycl::access::mode::read>(h);
    auto pB = buf_pre_B.get_access<sycl::access::mode::read>(h);
    auto pR = buf_pre_R.get_access<sycl::access::mode::read>(h);
    auto pO = buf_pre_O.get_access<sycl::access::mode::read>(h);
    auto pE = buf_pre_E.get_access<sycl::access::mode::read>(h);
    auto pXi_x = buf_Xi_x.get_access<sycl::access::mode::read>(h);
    auto pXi_y = buf_Xi_y.get_access<sycl::access::mode::read>(h);
    auto pEta_x = buf_Eta_x.get_access<sycl::access::mode::read>(h);
    auto pEta_y = buf_Eta_y.get_access<sycl::access::mode::read>(h);
    auto pU = buf_U.get_access<sycl::access::mode::read>(h);
    auto pV = buf_V.get_access<sycl::access::mode::read>(h);
    auto pW = buf_W.get_access<sycl::access::mode::read>(h);

    auto pNew_W = buf_new_W.get_access<sycl::access::mode::write>(h);
    ...
    h.parallel_for(sycl::range<2>(M, N), [=](sycl::id<2> idx) {
      ...
      if (j != 0) {
        ...
        pNew_W[i][j] =
            pW[i][j] +
            dt * (pW[i + 1][j] * (b_e + b_e1) + pW[index][j] * (b_w + b_w1) +
                  pW[i][j + 1] * (b_s + b_s1) + pW[i][j - 1] * (b_n + b_n1) -
                  2.0 / Re * (a + r) * pW[i][j] +
                  2.0 * b / Re *
                      (pW[i + 1][j + 1] - pW[i + 1][j - 1] - pW[index][j + 1] +
                       pW[index][j - 1]));
        pNew_W[M][j] = pNew_W[0][j];
      }
    });
  });
...
void psi_iteration() {
  ...
  while (1) {
    ...
    auto tas = calc_queue.submit([&](sycl::handler& h) {
      ...
      auto pW = buf_W.get_access<sycl::access::mode::read>(h);
      auto pPSI = buf_PSI.get_access<sycl::access::mode::read>(h);
      auto pNew_PSI = buf_new_PSI.get_access<sycl::access::mode::write>(h);
      ...
      h.parallel_for(sycl::range<2>(M, N), [=](sycl::id<2> idx) {
        ...
        if (j != 0) {
          double a = pA[i][j], b = pB[i][j], r = pR[i][j], o = pO[i][j],
                 e = pE[i][j];
          pNew_PSI[i][j] =
              ((a - o / 2.0) * pPSI[index][j] + (a + o / 2.0) * pPSI[i + 1][j] +
               (r - e / 2.0) * pPSI[i][j - 1] + (r + e / 2.0) * pPSI[i][j + 1] +
               2 * b *
                   (pPSI[i + 1][j + 1] - pPSI[i + 1][j - 1] -
                    pPSI[index][j + 1] + pPSI[index][j - 1]) -
               pW[i][j]) /
              (2 * a + 2 * r);
          pNew_PSI[M][j] = pNew_PSI[0][j];
        }
      });
    });
    tas.wait();
...
void velocity() {
  ...
  auto tas = calc_queue.submit([&](sycl::handler& h) {
    auto pPSI = buf_PSI.get_access<sycl::access::mode::read>(h);
    auto pJ = buf_J.get_access<sycl::access::mode::read>(h);

    auto pU = buf_U.get_access<sycl::access::mode::write>(h);
    auto pV = buf_V.get_access<sycl::access::mode::write>(h);

    auto pX_xi = buf_X_xi.get_access<sycl::access::mode::read>(h);
    auto pY_xi = buf_Y_xi.get_access<sycl::access::mode::read>(h);
    auto pX_eta = buf_X_eta.get_access<sycl::access::mode::read>(h);
    auto pY_eta = buf_Y_eta.get_access<sycl::access::mode::read>(h);

    h.parallel_for(sycl::range<2>(M, N), [=](sycl::id<2> idx) {
      int i = idx[0], index = i ? i - 1 : M - 1;
      int j = idx[1];
      if (j != 0) {
        pU[i][j] = pX_xi[i][j] / pJ[i][j] * (pPSI[i][j + 1] - pPSI[i][j - 1]) /
                       2.0 / d -
                   pX_eta[i][j] / pJ[i][j] * (pPSI[i + 1][j] - pPSI[index][j]) /
                       2.0 / d;
        pV[i][j] = pY_xi[i][j] / pJ[i][j] * (pPSI[i][j + 1] - pPSI[i][j - 1]) /
                       2.0 / d -
                   pY_eta[i][j] / pJ[i][j] * (pPSI[i + 1][j] - pPSI[index][j]) /
                       2.0 / d;
      }
    });
  });
...
void init_breakpoint(int index1) {
  char filename[30];
  sprintf(filename, "W%d.txt", index1);
  readPortfolio(W, filename);
  sprintf(filename, "U%d.txt", index1);
  readPortfolio(U, filename);
  sprintf(filename, "V%d.txt", index1);
  readPortfolio(V, filename);
  sprintf(filename, "PSI%d.txt", index1);
